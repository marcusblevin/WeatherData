...
*                                  
* 03/14/2018  ML		 ????	   Modified to return a HashMap of the each line instead
* 								   of writing to a file                                 
*
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */

package noaa; // ADDED ML
...
//  Fields making up the AW1-4 elements
    ...
    //public static void main(String[] args)				// Modified ML
    public HashMap<String, String> parseLine(String line)	// Added ML
    {
    	
    	HashMap<String, String> hm = new HashMap<String,String>();		// Added ML
 ...
 */
        ...
        try
        {
            ...
            */
            ...
            try
            {
                //String line     = null;		// commented out ML
                String lineHold = null;

                //while (( line = fInReader.readLine()) != null)
                if (!line.isEmpty()) 			// Added ML
                {
                    iCounter++;
//                    iOffset         = 25;
                    iLength         = line.length();
...
//                    if (iAT1_IndexOf > 0)
                    if (sCDS_Source.equals("O"))
                    {
                        ...
                        //continue;					// commented out ML
                        return null;				// Added ML
                    }
...
// Build Mandatory Data Section + the rest of the record
                    ...
                    //fFixedWriter.write(sOutputRecord+"\n");                 // Write out the record		// commented out ML
                    
                    // Added ML
                    if (sMDS_Temp.startsWith("*")) {
                    	return null; // if there is no temperature, don't bother including the record
                    }
                    
                    hm.put("USAF", sCDS_ID.trim());
                    hm.put("WBAN", sCDS_Wban.trim());
                    hm.put("timestamp", sCDS_Year+sCDS_Month+sCDS_Day+sCDS_Hour+sCDS_Minute);
                    hm.put("DIR", sMDS_Dir.trim());
                    hm.put("SPD", sMDS_Spd.trim());
                    hm.put("GUS", sOC1_Gus.trim());
                    hm.put("CLG", sMDS_Clg.trim());
                    hm.put("SKC", sGF1_Skc.trim());
                    hm.put("L", sGF1_Low.trim());
                    hm.put("M", sGF1_Med.trim());
                    hm.put("H", sGF1_Hi.trim());
                    hm.put("VSB", sMDS_Vsb.trim());
                    hm.put("MW1", sWW1234[3].trim());
                    hm.put("MW2", sWW1234[2].trim());
                    hm.put("MW3", sWW1234[1].trim());
                    hm.put("MW4", sWW1234[0].trim());
                    hm.put("AW1", sAW1234[3].trim());
                    hm.put("AW2", sAW1234[2].trim());
                    hm.put("AW3", sAW1234[1].trim());
                    hm.put("AW4", sAW1234[0].trim());
                    hm.put("W", sAY1_Pw.trim());
                    hm.put("TEMP", sMDS_Temp.trim());
                    hm.put("DEWP", sMDS_Dewp.trim());
                    hm.put("SLP", sMDS_Slp.trim());
                    hm.put("ALT", sMA1_Alt.trim());
                    hm.put("STP", sMA1_Stp.trim());
                    hm.put("MAX", sMaxTemp.trim());
                    hm.put("MIN", sMinTemp.trim());
                    hm.put("PCP01", sPcp01.trim()+sPcp01t.trim());
                    hm.put("PCP06", sPcp06.trim()+sPcp06t.trim());
                    hm.put("PCP24", sPcp24.trim()+sPcp24t.trim());
                    hm.put("PCPXX", sPcp12.trim()+sPcp12t.trim());
                    hm.put("SD", sAJ1_Sd.trim());

                }  // while read

            }
            catch (Exception ex) { 				// Added ML
            	ex.printStackTrace();
                System.exit(2);
            }
            /* commented out ML
